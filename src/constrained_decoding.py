...
class WhiteTokensProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores):
        # Mask scores not in white tokens
        mask = torch.full_like(scores, False, dtype=torch.bool)
        mask[:, list(self.white_token_ids)] = True
...
class ConstrainedGenerationMixin(GenerationMixin):
    def _beam_search(
        ...
        **model_kwargs,
        ...
        graph = generation_config.params["graph"]
        ...
        while self._has_unfinished_sequences(
            ...
            for i, p in enumerate(processors):
                t = time.time()
                state = graph.match(input_ids[i])
                time_match.append(time.time() - t)
                t = time.time()
                whitelist = graph.transitions(state)
                time_transitions.append(time.time() - t)
            ...
            for beam in range(num_beams):
                ...
                time_processors.append(time.time() - t)
        ...
        data = [time_match, time_transitions, time_processors]
        for i, section in enumerate(["Matching", "Transitions", "Logits Processors"]):
            mean = np.mean(data[i])
            std = np.std(data[i])
            print(f"{section}: Mean = {mean:.4f}, Std = {std:.4f}")

        # box of each time side to side
        plt.figure(figsize=(8, 6))
        plt.violinplot(data, showmeans=True, showextrema=True, widths=0.8)
        plt.xticks(
            [1, 2, 3],
            ["Matching", "Transitions", "Logits Processors"],
            fontsize=12,
            fontweight="bold",
        )
        plt.title("Time Overhead", fontsize=14)
        plt.xlabel("Code section", fontsize=14)
...
def constrained_decoding(checkpoint, input_str, params, nbeams=2):
    ...
    print("\n\n** Token Sequence **\n", output.sequences[0][len(inputs[0]) :], "\n")
